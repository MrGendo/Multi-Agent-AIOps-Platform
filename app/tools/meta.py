...
TOOL_META: Dict[str, ToolMeta] = {
    ...
    "execute_python_script": ToolMeta(
        read_only=False, # 可以产生副作用，例如修改文件或请求网络
        concurrency_safe=False,
        max_result_chars=8000,
        risk_level="high", # 动态代码执行属于高危操作
        search_hint="python script sandbox 执行代码",
    ),
    "list_available_secrets": ToolMeta(
        read_only=True,
        concurrency_safe=True,
        max_result_chars=2000,
        risk_level="low",
        search_hint="secrets passwords credentials 密码 机密 账号",
    ),
