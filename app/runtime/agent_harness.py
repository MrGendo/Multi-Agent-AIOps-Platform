...
4. 只选菜单中存在的 skill_name, 不要编造。原则上只选择最核心的 1 个专家，除非明确涉及多个不同领域（如数据库与网络同时异常），才允许多选（避免多个专家执行重复的诊断操作）。
5. 故障域映射:
...
如果属于, 从菜单中选一个 skill_name；结合历史经验参考，如果有类似的历史故障处理经验，优先选择相关的专家进行排查；如果不能确定或没有合适项, 选 `{generic}` 兜底。
请注意：为了避免计算资源浪费，绝大多数单点故障（如内存高、进程异常）只能选择 1 个专家。只有当故障明确跨越多个架构层级时，才返回多个专家。
"""
...
# 工作原则
1. 如果可以通过系统内置的监控、日志工具获取真实数据, 必须优先调用内置工具。
2. 【高危警告】只有当你用尽了系统内置工具，依然无法拿到需要的指标或日志时，才允许使用 `execute_python_script` 动态编写代码进行探测。严禁在已有对应工具的情况下重复造轮子！
3. 【机密安全警告】如果你动态编写的代码需要连接任何需要密码的系统（如 MySQL, Redis），请先调用 `list_available_secrets` 工具查看当前支持注入哪些机密的 Key，然后在代码中通过 `os.environ.get('SECRET_XXX')` 的方式获取（必须带上 SECRET_ 前缀）。绝对不允许在代码中硬编码任何明文密码或猜测的凭据！
4. 动态执行代码时，请保持代码简短、只读，避免任何破坏性操作。
5. 工具返回结果后, 用 3-5 句话总结关键信息。
6. 只完成当前步骤, 不要越界规划下一步。
7. 始终使用中文输出。
8. 【无人值守】这是全自动诊断流程, 没有人会回答你的问题。严禁反问用户「你遇到了什么问题/请提供更多信息」——用户输入就是告警的全部信息, 信息不足时基于现有信息推理并明确标注假设; 工具查不到数据时报告「工具不可用/无数据」并继续, 绝不停下来等人工输入。
9. 【诚实报告负结果】探测工具返回「连接被拒绝 / 域名无法解析 / 服务不存在」时, 如实原样报告该负结果, 严禁把「连不上」脑补成「目标内部故障」或编造解释; 多个探测都连不上时, 明确写出「目标疑似不存在」。
...
3. 【极其重要】严禁重复：绝对不允许把“已完成步骤”中刚做过的步骤再次写进剩余计划里！如果你觉得没有新的排查手段了，必须设置 is_finished=true，不要重复执行同一个工具。
...
7. 【目标真伪】如果多个步骤的探测一致返回「连接被拒绝 / 域名无法解析 / 服务不存在」等**目标缺失**信号:
   优先结论是「诊断目标疑似不存在或不可达」, 而不是围绕它编造内部故障根因。此时建议用户核对地址/部署状态,
   并在报告关键证据里如实列出这些负向探测结果。绝不把「查不到数据」脑补成「目标内部出了某故障」。
...
# 新摘要"""
        ...
        return [
            {"role": "system", "content": self._SKILL_ROUTER_SYSTEM_PROMPT},
            {
                ...
                "content": self._SKILL_ROUTER_USER_TEMPLATE.format(
                    menu=menu,
                    experience_context=experience_context or "(无相似历史经验)",
                    input=user_input,
                    generic=generic,
                ),
            },
    ...
    def planner_fallback_plan(self, reason: str) -> list[str]:
        ...
        # 兜底计划 (LLM 挂了才走到): 不放「查询知识库」类依赖外部服务的步骤 —
        # LLM 已不可用时知识库大概率也不可用, 死步骤只会让 Executor 反问用户.
        # 改为纯推理步骤, 让 Executor 基于原始告警直接产出结论.
        return [
            "基于原始告警信息与已知的运维常识, 推理最可能的根因并直接给出诊断结论",
            "汇总现有信息, 给出诊断结论",
        ]
    ...
    def build_executor_task_prompt(
        ...
        *,
        ...
        critic_warning = ""
        if critic_feedback:
            critic_warning = f"\n# 🚨 评审员(Critic)驳回警告\n你上一次的执行结果被评审员打回重做，理由是：\n{critic_feedback}\n请修正参数、修补脚本代码，或实际调用相关工具后重新执行！\n"
