...
async def replan_node(state: PlanExecuteState) -> PlanExecuteState:
    ...
    # 强制去重防呆：如果 LLM 把刚才刚执行完的步骤又原样吐出来了，强制干掉
    if new_plan and past_steps:
        last_executed_step = past_steps[-1][0].strip().lower()
        filtered_plan = []
        for step in new_plan:
            # 如果新计划里的步骤和刚刚执行的步骤高度雷同（前 20 个字符一样），则剔除
            if step.strip().lower()[:20] == last_executed_step[:20]:
                logger.warning(f"[Replanner] 发现 LLM 重复生成刚刚执行过的步骤: {step[:30]}..., 已强制剔除")
                continue
            filtered_plan.append(step)
        new_plan = filtered_plan

    if not new_plan:
        logger.warning("[Replanner] 标记 not finished 但新计划为空 (或被去重), 兜底生成报告")
